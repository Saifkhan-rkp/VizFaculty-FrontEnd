...
export default function Register() {
  ...
  const muteFunc = async (data) => {
    setSubmitting(true)
    return await axios.post(`${process.env.REACT_APP_API_KEY}/api/auth/register`, data);
  }
  ...
  const onSubmit = (data) => {
    const registerData = { ...data, role: 'normal' };
    mutate(registerData);
  };
  return (
    <>
          ...
          <div className="w-full lg:w-6/12 px-4">
            ...
            {regResponse && <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-slate-200 border-0">
              ...
              <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
                ...
                <p className="text-justify">Hello <strong>{response?.user.name}</strong> please check your email inbox verification email has been sent to <b>{response?.user.email}</b>. Please verify your email before login; verification link will be valid for 3 hours.</p>
                <br />
